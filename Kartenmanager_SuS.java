...
public class Kartenmanager_SuS extends Ereignisanwendung {
    ...
    {
        for (int i = 1; i < kartenAnzahl - 1; i++) 
        {
            // Minimum im Bereich i..(kartenAnzahl-1) suchen
            int minIndex = findeMinimumkartewert(i, kartenAnzahl - 1);

            // Jetzt die Karte an Position minIndex "herausnehmen" ...
            karten.geheZuPosition(minIndex + 1); 
    ...
    private int findeMinimumkartewert(int start, int end){
        if (start==end){ //Basisfall damit alle unsortierten Karten darauf geprüft werden ob sie den kleinsten Wert haben, es soll keine Karte übersehen werden
            return start;
        }
        int minKarteRest = findeMinimumkartewert(start + 1, end);//Rekursion wird wieder aufgerufen

        // Vergleiche die aktuelle Karte mit der Minimumkarte des Rests
        karten.geheZuPosition(start+1); 
        aktuell = karten.aktuelles();
        int aktuellerWert = aktuell.wert;//Zahl und Farbe der Karte werden zu einer Zahl verrechnet, sodass nur einmal sortiert werden muss
        karten.geheZuPosition(minKarteRest);
        aktuell = karten.aktuelles();
        int minWertRest = aktuell.wert;//Zahl und Farbe der Karte werden zu einer Zahl verrechnet für das Minimum, sodass nur einmal sortiert werden muss

        return (aktuellerWert < minWertRest) ? start : minKarteRest; //Aktueller Wert wird mit der bisher kleinsten Karte verglichen, der kleinere Wert wird zurückgegeben, wenn sie gleich sind, wird die bis dahin kleinste Karte zurückgegeben
